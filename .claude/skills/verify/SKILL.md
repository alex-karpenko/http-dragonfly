...
Steps 2-5 mirror exactly what `.github/workflows/ci.yaml` runs on every PR. Step 6 runs the local dev-workflow gate from `.pre-commit-config.yaml` (trailing whitespace, check-yaml/toml/json, large-file/secret detection, plus `cargo fmt --` and `cargo check` again).
